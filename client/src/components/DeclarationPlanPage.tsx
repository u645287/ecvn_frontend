...
export default function DeclarationPlanPage() {
  ...
  const summaryOption = useMemo<EChartsOption>(
    () => ({
      ...
      series: [
        { name: '發電 (kW)', type: 'line', smooth: true, showSymbol: true, symbol: 'circle', symbolSize: 6, color: '#0ea5e9', lineStyle: { width: 2.2, color: '#0ea5e9' }, itemStyle: { color: '#0ea5e9' }, data: summaryRows.map((d) => d.gen) },
        { name: '用電 (kW)', type: 'line', smooth: true, showSymbol: true, symbol: 'circle', symbolSize: 6, color: '#ef4444', lineStyle: { width: 2.2, color: '#ef4444' }, itemStyle: { color: '#ef4444' }, data: summaryRows.map((d) => d.load) },
        { name: '合約數量', type: 'line', smooth: true, showSymbol: true, symbol: 'circle', symbolSize: 6, color: '#16a34a', lineStyle: { width: 2.2, color: '#16a34a', type: 'dashed' }, itemStyle: { color: '#16a34a' }, areaStyle: { color: 'rgba(22,163,74,0.65)' }, data: summaryRows.map((d) => d.contractQty) },
  ...
  const contractOption = useMemo<EChartsOption>(
    () => ({
      ...
      yAxis: { ...(baseChartOption.yAxis as object), name: 'kW' },
      series: [
        { name: '再生能源合計（參考）', type: 'line', smooth: true, showSymbol: true, symbol: 'circle', symbolSize: 6, color: '#2563eb', lineStyle: { width: 2.2, color: '#2563eb' }, itemStyle: { color: '#2563eb' }, data: contractTransferRows.map((d) => d.genSum) },
        { name: '負載合計（上限）', type: 'line', smooth: true, showSymbol: true, symbol: 'circle', symbolSize: 6, color: '#dc2626', lineStyle: { width: 2.2, color: '#dc2626' }, itemStyle: { color: '#dc2626' }, data: contractTransferRows.map((d) => d.loadSum) },
        {
          name: '合約轉供量',
          type: 'line',
          smooth: false,
